...
static CommandLog buildTestCommandLog() {
    ...
    log.record(10, PlaceBuildingCmd{14, 15, BuildingType::Residence});
    log.record(20, PlaceBuildingCmd{16, 15, BuildingType::Lumberjack});
    log.record(30, PlaceBuildingCmd{15, 14, BuildingType::Fishery});
    log.record(40, PlaceBuildingCmd{15, 16, BuildingType::Residence});
    log.record(50, PlaceBuildingCmd{14, 14, BuildingType::Road});
    log.record(100, PlaceBuildingCmd{16, 16, BuildingType::Residence});
    log.record(200, DemolishBuildingCmd{16, 16});
    log.record(300, PlaceBuildingCmd{13, 15, BuildingType::Residence});
    log.record(400, PlaceBuildingCmd{17, 15, BuildingType::Lumberjack});
    log.record(500, PlaceBuildingCmd{13, 14, BuildingType::Road});
    log.record(600, PlaceBuildingCmd{17, 14, BuildingType::Road});
    log.record(1000, PlaceBuildingCmd{12, 15, BuildingType::Residence});
    log.record(1500, PlaceBuildingCmd{18, 15, BuildingType::Residence});
    log.record(2000, EvolveEraCmd{});
    log.record(2500, DemolishBuildingCmd{14, 14});
    log.record(3000, PlaceBuildingCmd{14, 14, BuildingType::Road});
    log.record(4000, EvolveEraCmd{});
    log.record(5000, PlaceBuildingCmd{16, 16, BuildingType::Residence});
    log.record(6000, EvolveEraCmd{});
    log.record(7000, DemolishBuildingCmd{13, 15});
    log.record(8000, PlaceBuildingCmd{13, 15, BuildingType::Residence});
...
TEST_CASE("Determinism - 10,000 Ticks Simulation Replay") {
    ...
    constexpr uint64_t kReferenceHash10k = 14108040256538311596ULL;
